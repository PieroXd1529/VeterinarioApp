<?php
	...
	public static function getConnectionPDO(){
		if( self::$cnPDO == null){
			try{
				$url = 'mysql:host=localhost;dbname=vetedog';
				$user = 'root';
				$pass = '';
	...
	class adminDAO{
		...
		public function allBitacora(){
			try{
				...
				$sql = "SELECT id_compra, fecha, compra.estado,provedor.id_prove, provedor.nomprove, provedor.nomprove, provedor.direcc, total, tipoc, tipopa FROM compra INNER JOIN provedor ON compra.id_prove = provedor.id_prove WHERE compra.estado='1'  ORDER BY id_compra DESC";
		...
		public function buscarAllBitacoraFecha($desde,$hasta){
			try{
				...
				$sql = 'SELECT compra.id_compra, compra.fecha,compra.estado ,provedor.id_prove, provedor.nomprove, provedor.nomprove, provedor.direcc, compra.total, compra.tipoc, compra.tipopa FROM compra INNER JOIN provedor ON compra.id_prove = supplier.id_prove WHERE  provedor.fecha BETWEEN "'.$desde.'" AND "'.$hasta.'" ORDER BY id_compra DESC';
