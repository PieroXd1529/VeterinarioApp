...
<div class="col-sm-4">
                                    <label class="control-label">Raza de mascota<span class="text-danger">*</span></label>
...
<?php 
 ...
 $stmt = $dbcon->prepare('SELECT * FROM raza');
        $stmt->execute();
        ...
        while($row=$stmt->fetch(PDO::FETCH_ASSOC))
        {
            extract($row);
            ?>
            <option value="<?php echo $id_raza; ?>"><?php echo $nomraza; ?></option>
            <?php
        }
        ?>
            </select> 
